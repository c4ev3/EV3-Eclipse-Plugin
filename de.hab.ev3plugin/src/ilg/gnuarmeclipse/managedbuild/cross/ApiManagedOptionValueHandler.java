...
public class ApiManagedOptionValueHandler extends ManagedOptionValueHandler {
	...
	public boolean handleValue(IBuildObject configuration,
		...
		if (event == EVENT_OPEN) {
			...
			String path = ProjectStorage.getValue(config, "api");
			if (path.length() > 0) {
			...
			IConfiguration config = Utils.getConfiguration(configuration);
			ProjectStorage.putValue(config, path, "api");
